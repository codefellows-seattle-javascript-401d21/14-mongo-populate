...
describe('Route Testing', () => {
  this.mockStar = { starName: 'test star put' };
  ...
  describe('PUT /api/v1/star', () => {
    ...
    describe('Valid Routes/Data', () => {
      ...
      it('Should respond with a single star', () => {
        return superagent.get(`${api}/${this.mockStar._id}`)
          .then(res => {
            expect(JSON.parse(res.text).starName).toBe('updated');
          });
      });
    });
    describe('Invalid Routes/Data', () => {
      it('Should respond a validation error response if a file id does not match the id sent', () => {
        return superagent.put(`${api}/${this.mockStar._id.slice(0, -1)}9`)
          .send(this.mockStar)
          .catch(err => {
